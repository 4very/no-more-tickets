...
async def login(login: authLoginBody) -> authLoginResponse | HTTPErrorType:
    try:
        ...
        user = pauth.sign_in_with_email_and_password(
            login.email, login.password.get_secret_value()
        )
        return user
        ...
        if error_code == "INVALID_PASSWORD":
            return JSONResponse(
                status_code=400,
                content={"msg": "Password is invalid!", "type": error_code},
            )

        if error_code == "USER_DISABLED":
            return JSONResponse(
                status_code=400,
                content={"msg": "User's account is disabled!", "type": error_code},
            )
