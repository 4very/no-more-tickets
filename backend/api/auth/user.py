...
async def user(token: str) -> HTTPError | Any:
    ...
    # firebase_admin already does this error checking so we can just leverage that
        ...
        return JSONResponse(
            status_code=400, content={"msg": "Invalid token", "type": "INVALID_TOKEN"}
        )

    # token is expired and needs to be refreshed
    except ExpiredIdTokenError:
        return JSONResponse(
            status_code=400, content={"msg": "Expired token", "type": "EXPIRED_TOKEN"}
        )

    # token has been revoked
    except RevokedIdTokenError:
        return JSONResponse(
            status_code=400, content={"msg": "Revoked token", "type": "REVOKED_TOKEN"}
        )

    # else for revoked and expired, includes malformed or other bad statuses
    except InvalidIdTokenError:
        return JSONResponse(
            status_code=400, content={"msg": "Malformed token", "type": "BAD_TOKEN"}
        )
    ...
    # user's account has been disabled
        ...
        return JSONResponse(
            status_code=400, content={"msg": "User disabled", "type": "USER_DISABLED"}
        )
